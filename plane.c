...
void HariMain(void)
{
	...
	for (;;) {
		...
		for(e = 0; e<8;e++)
		{
			api_putstrwin(win,enemy[e].x, enemy[e].y, 0, 1, "@"); //ÓÃºÚÉ«¸²¸Ç¹ì¼£ 
			enemy[e].y = enemy[e].y+5;	  //move down
			api_putstrwin(win,enemy[e].x, enemy[e].y, 3, 1, "@");  //ÏÔÊ¾ÐÂÎ»ÖÃ 
			...
			if(planex >enemy[e].x-20 && planex <enemy[e].x+3)            //ÅÐ¶ÏÅö×² 
...
void move(int win ,int i, int timer, char *keyflag)
{
	...
	for (;;) {
		...
		if (j == '4') {
			keyflag[0 /* left */]  = 1;
	    	if (planex > 5) {api_putstrwin(win, planex, planey, 0, 3, "^$^"); planex -= 5; }  //Çå³ý¹ì¼£ 
		}
		if (j == '6') {
			keyflag[1 /* right */] = 1;
